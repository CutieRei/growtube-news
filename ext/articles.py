...
news_types: Dict[str, Tuple[str, Color, int]] = {
    ...
    "growtopia announcement": (
        "Growtopia Announcement",
        from_hex("#67e85f"),
        Channel.category1,
    ),
    "growtube announcement": (
        "Growtube Announcement",
        from_hex("#ffffff"),
        Channel.category1,
    ),
...
server_status: Dict[str, Tuple[str, str, Color]] = {
    ...
    "Maintenance": (
        ...
        "<:gtbusy:874224931230060604>",
        Color.red(),
    ),
    "Extended Maintenance": (
        "The maintenance is extended",
        "<:gtaway:874224931389472778>",
        Color.gold(),
    ),
}
...
async def _setchannel(
    ctx: commands.Context, chtype: int, channel_id: int, webhook_id: int
) -> bool:
    ...
    await ctx.send(
        f"Successfully set **{_channel_dict[chtype]}** channel to {channel.mention}"
    )
    return True
...
def check(ctx: commands.Context):
    if not ctx.guild:
        ...
        return True
    raise NotPermittedForPublish(
        "You are not permitted to access the news configuration!"
    )
...
class ServerView(discord.ui.View):
    def __init__(
        self, ctx: commands.Context, *, timeout: Optional[float] = None
    ) -> None:
    ...
    async def extended_maintenance(
        self, button: discord.ui.Button, interaction: discord.Interaction
    ) -> None:
        if self.view_check(interaction):
...
class Articles(commands.Cog):
    ...
    async def maintenance(self, ctx):
        ...
        embed = Embed(color=embed_color, description="What's the status of the server?")
        ...
        embed.set_author(
            name=stype[0],
            icon_url="https://cdn.discordapp.com/emojis/{}".format(
                stype[1].split(":")[-1].replace(">", "")
            ),
        )
        embed.set_footer(
            text=f"Growtube News | Published by {ctx.author}", icon_url=emoji_url
        )
    ...
    async def set_announcement(
        self, ctx: commands.Context, channel: TextChannel, webhook: Optional[int] = None
    ):
        """
    ...
    async def publish(self, ctx):
        ...
        try:
            ...
            ntype = await message_wait(
                ctx,
                lambda x: news_types.get(x.content.lower()),
                "What is the news type? (`{}`)".format(
                    ", ".join(
                        (i.title() if len(i) > 4 else i.upper() for i in news_types)
                    )
                ),
                "Invalid type!",
            )
            ntype, color, chtype = news_types[ntype.content.lower()]
            body = await message_wait(
                ctx, None, "And what would be the body of the news?"
            )
            image = await message_wait(
                ctx, None, "Please insert an image link (or N/A if there's none)"
            )
            image = image.content if image.content.lower() != "n/a" else None
            link = await message_wait(
                ctx, None, "Please insert a link (or N/A if there's none)"
            )
        ...
        confirmation = await self.bot.wait_for(
            ...
            and ctx.channel == x.channel
            and x.content.lower() in {"yes", "y", "no", "n"},
        )
